...
export async function fetchTradingHistory(
  ...
  while (true) {
    ...
    const trades = batch.filter((item: any) =>
      item.type === 'TRADE' || item.type === 'REDEEM'
    );
 ...
 */
export async function fetchMarket(conditionId: string): Promise<MarketData | null> {
  try {
 ...
 */
async function enrichRedeemOutcomes(trades: PolymarketTrade[]): Promise<PolymarketTrade[]> {
  const redeems = trades.filter(t => t.type === 'REDEEM' && !t.outcome);

  if (redeems.length === 0) return trades;
