...
async function generateDetailedMode(report, options, PDFDocument) {
    ...
    // Part I - Short Term
    if (report.shortTerm.length > 0) {
        await fillDetailedPart(pdfDoc, form, report.shortTerm, report.shortTermSummary, 'Part1', 'f1', 'c1_1');
    }
    // Part II - Long Term
    if (report.longTerm.length > 0) {
        await fillDetailedPart(pdfDoc, form, report.longTerm, report.longTermSummary, 'Part2', 'f2', 'c2_1');
    }
    form.flatten();
    const pdfBytes = await pdfDoc.save();
...
async function fillDetailedPart(pdfDoc, form, entries, summary, partName, fieldPrefix, checkboxName) {
